...
#endif
extern void simobuck_force_active(void);
extern void simobuck_force_disable(void);
...
am_hal_sysctrl_sleep(bool bSleepDeep)
{
    ...
    bool bSimobuckForcedToActive, bBuckIntoLPinDS;
    bool bRevertNvmDeepSleep = false;
    uint32_t ui32VDDCsave = 0xFFFFFFFF;

    bSimobuckForcedToActive = bBuckIntoLPinDS = false;
         ...
         (MCUCTRL->DBGCTRL_b.DBGTPIUENABLE == MCUCTRL_DBGCTRL_DBGTPIUENABLE_DIS) )
    {
        ...
        //
        // Check if we can enter SIMOBUCK LP mode during deepsleep. If not,
        // keep SIMOBUCK active.
        //
        if ( !(MCUCTRL->VRCTRL_b.SIMOBUCKOVER) )
        {
            if ( PWRCTRL->DEVPWRSTATUS_b.PWRSTADC || PWRCTRL->AUDSSPWRSTATUS_b.PWRSTAUDADC )
            {
                 ...
                 PWRCTRL_DEVPWRSTATUS_PWRSTIOS_Msk)) )
        {
            ...
            // Turn off coreldo and memldo parallel operation during deepsleep to save power
            // coreldo and memldo together consumes ~15uA@1.9V.
            //
            // ################################################################################
            // #### INTERNAL END ####
    ...
    // vulnerability.  However, if we can't guaranteed the time between enabling
